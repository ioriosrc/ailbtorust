...
impl Component for VisibilityToggle {
    ...
    fn update(&mut self, msg: Self::Message) -> EffectivelyChanged<Self> {
        match msg {
            () => EffectivelyChanged::NoChanges,
        }
    }

    fn change(&self, new_props: CheckboxProps) -> bool {
    ...
    fn view(&self) -> Html<Self> {
        let classes = use_state(|| {
            let mut classes = Classes::new();
            if self.props.size == "small" {
                classes.push("VisibilityToggle-size-small");
            }
            classes
        });

        let handle_change = move |event: MouseEvent| {
            let checked = event.target().is_truthy();
            self.props.on_change(event, checked);
        };
